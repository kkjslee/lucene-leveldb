 ...
 */
public class LeveldbInputStream extends IndexInput {
    ...
    private final LeveldbFileStore store;

    private final String name;

    public LeveldbInputStream(String name, LeveldbFileStore store, int bufferSize) {
        this(name, store, bufferSize, store.getSize(name));
    }

    public LeveldbInputStream(String name, LeveldbFileStore store, int bufferSize, long length) {
        super("LeveldbInputStream(name=" + name + ")");
        this.name = name;
        this.store = store;
    ...
    public IndexInput slice(String sliceDescription, final long offset, final long length) throws IOException {
        ...
        return new LeveldbInputStream(name, store, bufferSize, offset + length) {
            {
                seek(0L);
            }
    ...
    public IndexInput clone() {
        LeveldbInputStream in = (LeveldbInputStream) super.clone();
